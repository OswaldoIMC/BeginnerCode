...
const CppScreen = () => {
  ...
  const navigation =
    useNavigation<StackNavigationProp<RootStackParamList, "Cpp">>();
  ...
  const loadCourseData = async () => {
    try {
      ...
      const cppCourse = DataService.getCourseById("cpp");
      setCourse(cppCourse || null);

      // Obtener las lecciones del curso
      const cppLessons = DataService.getLessonsByCourse("cpp");
      setLessons(cppLessons);

      // Cargar progreso de todas las lecciones
      const progressMap: { [key: string]: number } = {};
      for (const lesson of cppLessons) {
        const progress = await DataService.getLessonProgress("cpp", lesson.id);
      ...
      <ScrollView contentContainerStyle={styles.lessonList}>
        <View style={styles.contentContainer}>
          ...
          <View style={styles.headerContainer}>
            <Image source={CppImage} style={styles.logo} />
